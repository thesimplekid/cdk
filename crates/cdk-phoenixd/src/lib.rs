...
impl MintLightning for Phoenixd {
        ...
        Ok(futures::stream::unfold(
            ...
            |(mut receiver, phoenixd_api)| async move {
                match receiver.recv().await {
                    Some(msg) => {
                        let check = phoenixd_api.find_invoice(&msg.payment_hash).await;
    ...
    async fn pay_invoice(
        ...
        let pay_response = self
            .phoenixd_api
            .pay_bolt11_invoice(&melt_quote.request, partial_msats)
            .await?;

        let total_spent_sats = pay_response.recipient_amount_sat + pay_response.routing_fee_sat;
        ...
        Ok(PayInvoiceResponse {
            ...
            total_spent_msats: total_spent_sats * 1000,
        })
    }
    ...
    async fn create_invoice(
        ...
        Ok(CreateInvoiceResponse {
            ...
            request: create_invoice_response.serialized.parse()?,
        })
    }

    async fn check_invoice_status(&self, payment_hash: &str) -> Result<MintQuoteState, Self::Err> {
        let invoice = self.phoenixd_api.find_invoice(payment_hash).await?;
