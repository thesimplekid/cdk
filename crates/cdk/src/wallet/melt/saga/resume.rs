...
use cdk_common::wallet::{
    MeltOperationData, MeltSagaState, OperationData, ProofInfo, Transaction, TransactionDirection,
...
impl Wallet {
                ...
                Some(change_blinded_messages) if !change_blinded_messages.is_empty() => {
                        ...
                        .await
                    {
                        ...
                        Ok(OutputRecoveryResult::Unavailable) => {
                            tracing::warn!(
                                "Melt saga {} - couldn't restore change proofs; preserving \
                                 paid state and failing recovery until change can be recovered.",
                                saga_id
                            );
                            self.persist_paid_melt_recovery_state(
                                saga_id,
                                data,
                                quote_status,
                                &melt_input_proofs,
                            ...
                            .await?;
                            return Err(Error::Custom(format!(
                                "Melt saga {} is paid, but change proofs could not be recovered",
                                saga_id
                            )));
                        }
                        Err(e) => {
                ...
                _ => {
                    tracing::warn!(
                        "Melt saga {} - payment succeeded but no change blinded messages stored; \
                         preserving paid state and failing recovery until change can be recovered.",
                        saga_id
                    );
                    self.persist_paid_melt_recovery_state(
                        saga_id,
                    ...
                    .await?;
                    return Err(Error::Custom(format!(
                        "Melt saga {} is paid, but change recovery data is missing",
                        saga_id
                    )));
                }
            }
    ...
    async fn persist_paid_melt_recovery_state(
        &self,
        saga_id: &uuid::Uuid,
        data: &MeltOperationData,
        quote_status: &MeltQuoteStatusResponse,
        melt_input_proofs: &[ProofInfo],
    ) -> Result<(), Error> {
        let pending_proof_ys: Vec<_> = melt_input_proofs
            .iter()
            .filter(|proof| proof.state == State::Pending)
        ...
        if let Some(mut quote) = self.localstore.get_melt_quote(&data.quote_id).await? {
            quote.state = MeltQuoteState::Paid;
            quote.payment_proof = quote_status.payment_proof();
            self.localstore.add_melt_quote(quote).await?;
        } else {
            tracing::warn!(
                "Melt saga {} - paid quote {} not found while persisting recovery state",
                saga_id,
                data.quote_id
            );
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_recover_melt_paid_fails_when_change_recovery_data_is_missing() {
        let db = create_test_db().await;
        let mint_url = test_mint_url();
        let keyset_id = test_keyset_id();
        let saga_id = uuid::Uuid::new_v4();
        ...
        let err = result.expect_err("missing recovery data should fail recovery");
    ...
    #[tokio::test]
    async fn test_recover_incomplete_sagas_counts_paid_melt_with_definitive_restore_failure_as_failed(
    ) {
        let db = create_test_db().await;
        let mint_url = test_mint_url();
        let keyset_id = test_keyset_id();
