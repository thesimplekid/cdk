...
use cdk_common::{Amount, FinalizedMelt, MeltQuoteState};
...
impl Wallet {
    ...
    /// # Returns
    ///
    /// - `Ok(Some(FinalizedMelt))` - The melt was finalized (either paid or failed/unpaid)
    /// - `Ok(None)` - The melt was skipped (still pending) or compensated (never started)
    /// - `Err(e)` - An error occurred during recovery
    #[instrument(skip(self, saga))]
    pub async fn resume_melt_saga(&self, saga: &WalletSaga) -> Result<Option<FinalizedMelt>, Error> {
        ...
        match state {
            MeltSagaState::ProofsReserved => {
                ...
                // Return None since no payment was ever attempted
                tracing::info!(
