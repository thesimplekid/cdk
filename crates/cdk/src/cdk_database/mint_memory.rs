...
use cdk_common::database::{Error, MintAuthDatabase, MintDatabase};
use cdk_common::mint::MintKeySetInfo;
use cdk_common::nut00::ProofsMethods;
use cdk_common::{AuthProof, MintInfo};
...
impl MintMemoryAuthDatabase {
    ...
    pub fn new(
        ...
        Ok(Self {
            active_keyset: Arc::new(RwLock::new(active_keyset)),
            keysets: Arc::new(RwLock::new(
                keysets.into_iter().map(|k| (k.id, k)).collect(),
            )),
            proofs: Arc::new(RwLock::new(proofs)),
