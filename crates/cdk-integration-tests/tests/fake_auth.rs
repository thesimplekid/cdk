...
const OIDC_USER: &str = "test";
const OIDC_PASSWORD: &str = "test";
...
async fn test_mint_blind_auth() {
    ...
    top_up_blind_auth_proofs(wallet.clone(), 10, OIDC_USER, OIDC_PASSWORD).await;
...
async fn test_mint_auth_over_max() {
    ...
    let access_token = oidc_client
        .get_access_token_with_user_password(OIDC_USER.to_string(), OIDC_PASSWORD.to_string())
        .await
    ...
    let auth_proofs = wallet
        .mint_blind_auth((mint_info.nuts.nutxx1.expect("Auth enabled").bat_max_mint + 1).into())
        .await;
...
async fn test_melt_with_invalid_auth() {
    ...
    println!("{:#?}", proofs);
    let proof = proofs.first().expect("wallet hash one proof");
