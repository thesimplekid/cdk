    ...
    RM: DatabasePool + 'static,
{
   ...
   ;
    ...
    async fn add_mint_quote(&self, quote: MintQuote) -> Result<(), database::Error> {
        ...
        let query_str = if RM::Connection::name() == "postgres" {
            r#"
    INSERT INTO mint_quote
    (id, mint_url, amount, unit, request, state, expiry, secret_key, payment_method, amount_issued, amount_paid, estimated_blocks, payjoin, version, used_by_operation)
    VALUES
    (:id, :mint_url, :amount, :unit, :request, :state, :expiry, :secret_key, :payment_method, :amount_issued, :amount_paid, :estimated_blocks, (:payjoin)::jsonb, :version, :used_by_operation)
    ON CONFLICT(id) DO UPDATE SET
        mint_url = excluded.mint_url,
        amount = excluded.amount,
        unit = excluded.unit,
 ...
 (id, unit, amount, request, fee_reserve, state, expiry, payment_method, estimated_blocks, fee_index, payjoin, version, mint_url, used_by_operation)
 VALUES
 (:id, :unit, :amount, :request, :fee_reserve, :state, :expiry, :payment_method, :estimated_blocks, :fee_index, (:payjoin)::jsonb, :version, :mint_url, :used_by_operation)
 ON CONFLICT(id) DO UPDATE SET
     unit = excluded.unit,
     amount = excluded.amount,
     request = excluded.request,
